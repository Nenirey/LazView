...
uses
  ...
  LazUTF8{$IFDEF WINDOWS}, Registry, Windows{$ENDIF},
  BGRABitmapTypes, BGRABitmap, BGRAThumbnail, BGRAAnimatedGif,
  DateUtils, Math, ImgSize;
...
var
  Form1: TForm1;
  full,ifgif,startdraw,startselect:boolean;
  flist:TStringList;
...
begin
  ...
  begin
    (Form1.ScrollBox2.Controls[i] as tthumbimage).Width:=thumbsize;
    (Form1.ScrollBox2.Controls[i] as tthumbimage).Height:=thumbsize;
    (Form1.ScrollBox2.Controls[i] as tthumbimage).Left:=(thumbsize+5)*i;;
  end;}
end;
...
begin
  starttime:=Now();
  try
    Form1.Caption:='LazView '+SysToUTF8(fimagen);
    Form1.Label1.Caption:=inttostr(ifile+1)+'/'+inttostr(nfile)+'  '+SysToUTF8(fimagen);
    Form1.Label2.Caption:=inttostr(ifile+1)+'/'+inttostr(nfile)+'  '+SysToUTF8(fimagen);
    Form1.StatusBar1.Panels.Items[0].Text:=inttostr(ifile+1)+'/'+inttostr(nfile);
    Form1.StatusBar1.Panels.Items[3].Text:=' Tamaño:'+FloatTostr(Round(FileSize(fimagen)/1024))+'Kb';
    if restorezoom and (Form1.ToolButton24.Down=false) then
      zoomnormal()
    else
      zoomstrech();
    if UpperCase(ExtractFileExt(fimagen)) = '.GIF' then
    begin
      ifgif:=true;
    ...
    case UpperCase(ExtractFileExt(fimagen)) of
      '.GIF':
      begin
        BGRAgif:=TBGRAAnimatedGif.Create(UTF8ToSys(fimagen));
      ...
      begin
        streamimage:=TFileStream.Create(fimagen,fmOpenRead or fmShareDenyNone);
        bgcolor.alpha:=255;
        bgcolor.blue:=0;
        bgcolor.green:=0;
        bgcolor.red:=0;
        ...
        begin
          ...
          begin
            ...
            begin
              calculateaspectwidth(iw,ih,Form1.PairSplitterSide1.Height,tw);
              th:=Form1.PairSplitterSide1.Height;
            end
            else
            begin
              tw:=Form1.PairSplitterSide1.Width;
              calculateaspectheight(iw,ih,Form1.PairSplitterSide1.Width,th);
            end;
      ...
      '.ICO':
      begin
        Form1.Image1.Picture.LoadFromFile(fimagen);
        Form1.StatusBar1.Panels.Items[1].Text:='Resolucion:'+inttostr(Form1.Image1.Picture.Width)+'x'+inttostr(Form1.Image1.Picture.Height)+' '+zoomfactor(Form1.Image1.Picture.Width,Form1.Image1.Picture.Height,Form1.Image1.Width,Form1.Image1.Height)+'%';
        modethumb:=false;
        realimgwidth:=Form1.Image1.Picture.Width;
        realimgheight:=Form1.Image1.Picture.Height;
      ...
      '.PCX','.TGA','.PSD','.XWD':
      begin
        BGRAImage:=BGRABitmap.TBGRABitmap.Create(fimagen);
  ...
  begin
    ...
    begin
      ...
      Form1.ScrollBox2.HorzScrollBar.Position:=ifile*(thumbsize+5);
      //ShowMessage(inttostr(Form1.ScrollBox2.HorzScrollBar.Position));
      {for i:=0 to Form1.ScrollBox2.HorzScrollBar.Range do
      begin
        ShowMessage(inttostr(i));
        Form1.ScrollBox2.HorzScrollBar.Position:=i;
        ShowMessage(inttostr((Form1.ScrollBox2.Components[ifile] as TControl).));
        //if (Form1.ScrollBox2.Components[ifile] as TControl).Parent.BoundsRect.Left then
          //break;
      end;}
    end;
  end;
...
begin
  ...
  begin
    ...
    {Form1.ToolBar1.Transparent:=true;
    Form1.ToolBar1.ParentColor:=false;
  ...
  begin
    ...
    {Form1.ToolBar1.Transparent:=false;
    Form1.ToolBar1.Color:=clDefault;
    Form1.ToolBar1.ParentColor:=true;}
...
procedure loadfiles(place:UTF8string;fname:string);
var
   nombre:String;
   finfo:TSearchRec;
   contador:LongInt;
   flisttmp:TStringList;
   carpetatmp:UTF8String;
   nfiletmp:integer;
begin
  full:=false;
  fwidth:=Form1.Width;
  ...
  if FindFirst(carpetatmp+'*',faAnyFile,finfo)=0 then
  begin
    repeat
      Inc(contador);
      with finfo do
        ...
        begin
          case UpperCase(ExtractFileExt(Name)) of
            ...
            begin
              ...
              if (Name=fname) or (UpperCase(ExtractFileExt(Name))=UpperCase(ExtractFileExt(fname))) then
              begin
                flisttmp.Add(Name);
                Inc(nfiletmp);
              end;
...
begin
  ...
  for i:=0 to Form1.ShellTreeView1.Items.Count-1 do
  begin
...
begin
  ...
  begin
    if FileExists(ExpandFileName(Form1.SavePictureDialog1.FileName)) then
    ...
    begin
      case Form1.SavePictureDialog1.FilterIndex of
        ...
        begin
          ...
          Form1.Image1.Picture.Jpeg.SaveToFile(Form1.SavePictureDialog1.FileName);
        end;
        2:Form1.Image1.Picture.PNG.SaveToFile(Form1.SavePictureDialog1.FileName);
        3:Form1.Image1.Picture.Bitmap.SaveToFile(Form1.SavePictureDialog1.FileName);
        4:Form1.Image1.Picture.Icon.SaveToFile(Form1.SavePictureDialog1.FileName);
        5:
         begin
           if ifgif then
           begin
             BGRAGif.SaveToFile(Form1.SavePictureDialog1.FileName);
           end;
         end
      end;
    end;
...
begin
 ...
 thumbsize:=Form6.SpinEdit1.Value;
 refreshthumbs;
 ththumbs:=thumbsthread.Create(true);
...
begin
  ...
  begin
    ...
    streamimage:=TFileStream.Create(UTF8ToSys(carpeta+pathdelim+iname),fmOpenRead or fmShareDenyNone);
...
begin
  ...
  begin
    ...
    begin
      //ShowMessage('Creando thumb');
      thumbimages:=tthumbimage.Create(Form1.ScrollBox2);
      //thumbimages.BorderSpacing.Around:=10;
      thumbimages.Width:=thumbsize;
      thumbimages.Height:=thumbsize;
      //thumbimages.Top:=5;
      thumbimages.Left:=(thumbsize+5)*i;
  ...
  Form1.PairSplitter1.Position:=Form1.PairSplitter1.Height-thumbsize-40;
  Form1.PairSplitter1.Position:=Form1.PairSplitter1.Height-thumbsize-40+1;
      ...
      //try
        ...
        begin
          ...
          begin
            ...
            {$IFDEF WINDOWS}////Cause crash on linux
            try
            ...
            begin
              if (Form1.ScrollBox2.Controls[n].ClientOrigin.x+Form1.ScrollBox2.Controls[n].Width)>Form1.ClientOrigin.x then
                minv:=n
              else
                break;
            end;
            ...
            for n:=icenter to flist.Count-1 do
            begin
